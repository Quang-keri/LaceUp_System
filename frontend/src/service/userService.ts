...
export const userService = {
  ...
  addExtraPermissions: async (
    userId: string,
    permissionIds: number[],
  ): Promise<ApiResponse<UserResponse>> => {
    const response = await api.post(
      `${API_BASE_URL}/${userId}/extra-permissions`,
      permissionIds,
    );
    return response.data;
  },

  removeExtraPermissions: async (
    userId: string,
    permissionIds: number[],
  ): Promise<ApiResponse<UserResponse>> => {
  ...
  updateReputation: async (
    ...
    const response = await api.post(
      `${API_BASE_URL}/${userId}/reputation`,
      null,
      {
  ...
  getReputationLogs: async (
    ...
    page: number,
    size: number,
  ): Promise<ApiResponse<PageResponse<ReputationLogResponse>>> => {
    const params = { page, size };
    const response = await api.get(`/users/${userId}/reputation-logs`, {
      params,
    });
    return response.data;
  },
